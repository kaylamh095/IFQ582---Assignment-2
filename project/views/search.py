...
def search():
    ...
    if request.method == 'POST':
        ...
        search_term = ('%' + query + '%')
        print(f"Received search query: {query}")  # Debugging line to print the received search query
    ...
    return render_template('search.html', title=title, results=results, query=query, search_query=query)  ### pass the data to the search template to display # results= needs to match the jinja2 in search statement in search.html
### search_query=query, filter_category=filter_category, filter_access=filter_access, search_query=query,
